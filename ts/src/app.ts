...
class App {
            ...
            if (except.indexOf(js.client.data.command) > -1) {
                ...
                this.r_client.get(this._current_system + '_login_' + js.client.logintoken, (err, r) => {
                    if (err) {
                        ...
                        if (r) {
                            let res = JSON.parse(r) as IloginObj;
                            if (res.client.logintoken !== undefined) {
                                ...
                                this.r_client.get(this._current_system + '_usergui_' + js.client.logintoken, (err, r) => {
                                    if (err) {
                                        ...
                                        let res = JSON.parse(r) as IguiObj;
                                        if (res.gui) {
            ...
            if (err) {
                ...
                this.r_client.get(this._current_system + '_usergui_' + js.client.logintoken, (err, r) => {
                    if (err) {
                        ...
                        let res: IguiObj = JSON.parse(r) as IguiObj;
                        if (res.gui) {
    ...
    subscribe_online_chat(js) {
        ...
        try {
            ...
            this.findTagetByTargetId(msg.targetid).then(r => {
                let res: Array<ItargetObj> = r as Array<ItargetObj>;
                if (res.length) {
                    let target = res[0];
    ...
    login_ws(js) {
        ...
        try {
            this.authentication(js.client.data.user).then((res) => {
                let user: Igijuser = res as Igijuser;
    ...
    register_ws(js) {
        let deferred = Q.defer();
        js.client.data.user.system = new Array<ImySystem>();
    ...
    check_confirm_phone_ws(js) {
        ...
        this.r_client.get(this._current_system + '_phone_' + js.client.gui, (err, r) => {
            if (err) {
                ...
                if (r) {
                    let res: IphoneObj = JSON.parse(r) as IphoneObj;
    ...
    update_phone_ws(js) {
        ...
        this.r_client.get(this._current_system + '_phone_' + js.client.gui, (err, r) => {
            if (err) {
                ...
                if (r) {
                    let res = JSON.parse(r) as IphoneObj;
                    if (res.secret === js.client.data.secret) {
                        this.findUserByPhone(js.client.data.user.phonenumber).then((r) => {
                            let res: Igijuser = r as Igijuser;
                            if (res) {
                ...
                db.view(this.__design_view, 'findByParent', {
                    ...
                    if (err) deferred.reject(err);
                        ...
                        if (res.rows.length) {
                            let arr = [];
                            for (let index = 0; index < res.rows.length; index++) {
                                const element = res.rows[index].doc as Igijuser;
                                let e: any = {};
                                e.gui = element.gui;
                                e.username = element.username;
        ...
        try {
            this.r_client.get(this._current_system + '_online_' + client.username, (err, r) => {
                if (err) {
                    ...
                    if (r) {
                        let res = JSON.parse(r) as IonlineObj;
        ...
        this.r_client.get(this._current_system + '_forgot_' + js.client.gui, (err, res) => {
            if (err) {
                ...
                if (res) {
                    ...
                    this.findUserByPhone(js.client.data.user.phonenumber).then((res: Igijuser) => {
                        if (res) {
    ...
    checkUserPermission(username) {
        this.findUserByUsername(username).then((res: Igijuser[]) => {
            // console.log(res);
            if (res.length)
                return true;
            return false;
    ...
    change_password(js, phone, oldpass, newpass) {
        ...
        this.findUserByUsernameAndPhone(username, phone).then((res: Igijuser) => {
